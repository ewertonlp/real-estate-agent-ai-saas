...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se o login for bem-sucedido, o redirecionamento para o dashboard
      // já é tratado dentro da função `login` no AuthContext.
    } catch (err: any) {
      ...
      <div className="max-w-md w-full space-y-8 bg-card-light border border-button p-10 rounded-lg shadow-md">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-text">
            Ou{' '}
            <Link href="/register" className="font-medium text-button hover:text-hover">
              Crie uma nova Conta
            </Link>
          </p>
        </div>
